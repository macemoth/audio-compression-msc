...
def compress(infile):
    ...
    spectrogram = (20 * np.sqrt(spectrogram, out=np.zeros_like(spectrogram), where=(spectrogram != 0))).round()
    ...
    for i in tqdm(range(m)): # go frequency line by frequency line (works better than via frequencey)
        c = compress_tsc(np.column_stack([full_index, spectrogram[i, :]]), num_indices_to_keep="all")
        amplitudes.extend(bytes(c[:,1].astype(np.int8)))
    ...
    write_file(f"{fname}.tcad", header + cindexes + camplitudes)
...
def decompress(infile):
    ...
    amplitudes = np.zeros([m, n], dtype=np.int8)
    ...
    reconstructed_waveform = librosa.core.spectrum.griffinlim(np.square(amplitudes/20)) # Undo quantisation and STFT
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        ...
        print("Usage:")
        print("Compression: python3 tcad c <input>.wav")
        print("Decompression: python3 tcad d <compressed>.tcad")
    mode = sys.argv[1]
    infile = sys.argv[2]
    if mode not in ["c", "d"]:
